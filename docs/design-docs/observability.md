...
本地捕获发送通过同一普通模式记录搜索索引、投递门等待、同步设置读取、出站文件集合解析、文件元数据读取、
发送规划、图片内容哈希、图片载荷取出，以及 blob 发布的固定本地工作：明文哈希、压缩、分块加密、
本地 blob store 发布、引用保存和 ticket 生成。压缩与分块加密只在 `PublishBlobUseCase` 提供的不透明
本次发布作用域内记录，普通传输加密不冒充 blob 发布；底层 adapter 不读取或暴露条目、内容、摘要、
路径、tag 或 ticket。取消由观测守卫结算为中断，记录不得改变发布结果、等待顺序或重试责任。
