...
移动宿主使用 `MobileStartupLifecycle` 的方式相同：先创建一次性对象，再交给带 lifecycle 的构造入口；如果同时启用
产品分析，使用带 analytics 和 lifecycle 的组合入口。系统回调可以在构造调用尚未返回时从其他线程通知，构造返回后
同一对象仍可继续使用。每次重试必须创建新对象，不能把一次启动的控制对象交给第二次启动。
...
- `suspend()`：先停止接收操作，等待已取消调用实际退出，再释放节点、会话级后台任务、连接与 profile 安全租约；同时暂停进程级 Clipboard 磁盘工作，清除安全缓存并禁止后台重建。旧操作仍未退出时返回失败，不发布暂停成功。事件流与核心实例保持不变。
- `resume()`：先重新取得 profile 安全租约，再使用原持久数据和原设备身份重建节点与后台任务；文件仍被其他使用者占用时保持暂停，允许之后重试。不会自动重试暂停前被取消的操作。

以上不构成平台总停止时间保证。子运行期的停止预算尚未统一，平台绑定等待超时也不等于底层收尾完成；
完整移动后台安全验收见 [`后台暂停安全计划`](../exec-plans/active/2026-09-12-background-suspension.md)。
- `shutdown(deadline)`：停止操作、节点和全部后台任务，成功后发布 `Stopped` 并关闭事件流。
  收尾返回失败或等待超时则保持 `ShuttingDown` 和事件流，不报告已停止；再次调用同一入口可继续收尾，
  期间不能恢复或执行操作。进入 `ShuttingDown` 后，任务独立持有运行期直至实际收尾结束，等待超时或丢弃 Engine
  不取消该任务；再次关闭等待当前收尾，已成功关闭则直接成功。排队等待计入本次调用期限，不重置正在执行的预算。
  接受关闭后立即进入 `ShuttingDown`，操作排空也由该任务持有；宽限结束后取消剩余操作，确认其实际退出后才关闭运行期。
  会话操作超过期限仍未退出时保留旧会话及其依赖，不能把“已通知取消”当作“已经退出”。进程级受管任务超过优雅期限后会被强制结束并等待其实际析构；这类超时结果表示未优雅结束，不表示仍有受管任务继续占用本地资源。
  无法表示的极大期限返回 `InvalidInput`，不改变运行状态。实际退出可能超过宿主等待期限，不提前报告停止成功。
...
`QueryEntryDelivery` 由核心合并记录来源、当前可信设备集合和已发生的投递事实。来源明确区分本机、远端和无法追溯的历史记录；每个目标明确区分待处理、已送达、重复、不可达和失败。待处理既可以来自尚未接受发送的当前设备差集，也可以来自发送开始前已经持久保存、等待最终结果的恢复依据；两者对宿主保持同一状态。正常结果可以携带设备显示名和失败补充说明供界面展示，但这些内容不得进入调试输出或日志。记录不存在返回 `NotFound`，存储失败只返回脱敏错误。
