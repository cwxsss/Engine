...
impl PrepareSponsorCommitPort for DefaultSponsorCommitPreparation {
    async fn prepare(
        ...
        observe_local_result(LocalWorkStep::SponsorPrepareCommit, async {
            ...
            history
                ...
                .map_err(|error| PrepareSponsorCommitError::invalid(anyhow::Error::new(error)))?;
            let committed_history = AdmissionSignedMembershipHistory::from_bytes(
                history.encode_persisted_v2().map_err(|error| {
                    PrepareSponsorCommitError::unavailable(anyhow::Error::new(error))
                })?,
            )
            .map_err(|error| PrepareSponsorCommitError::invalid(anyhow::Error::new(error)))?;

            let staged: SponsorCandidateStagedV1 = postcard::from_bytes(
                preparation.staged_security().as_bytes(),
            )
            .map_err(|error| PrepareSponsorCommitError::invalid(anyhow::Error::new(error)))?;
            ...
            let sealed_recovery = AdmissionSealedRecoveryMaterial::from_bytes(
                staged.sealed_recovery_material.clone(),
            )
            .map_err(|error| PrepareSponsorCommitError::invalid(anyhow::Error::new(error)))?;
            ...
            let commit_body = AdmissionCommitV1::new(
                ...
                AdmissionSignedMembershipHistory::from_bytes(committed_history.as_bytes().to_vec())
                    .map_err(|error| {
                        PrepareSponsorCommitError::invalid(anyhow::Error::new(error))
                    })?,
                sealed_recovery,
            );
