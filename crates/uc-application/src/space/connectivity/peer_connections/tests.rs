...
fn dependencies() -> (Arc<Scope>, Arc<PeerReachability>) {
    ...
    let peer_reachability = Arc::new(PeerReachability {
...
async fn complete_target_operation_gets_twenty_seconds_but_never_more() {
    for completes in [true, false] {
        let (owner, _, peer_reachability) = fixture();
        peer_reachability.blocked.store(true, Ordering::SeqCst);
        peer_reachability.reachable.store(true, Ordering::SeqCst);
        owner.start().await;
        settle().await;
        ...
        assert!(
            !refresh.is_finished(),
            "the outer budget cancelled a valid complete target operation early"
        );
        assert_eq!(peer_reachability.calls.load(Ordering::SeqCst), 1);
        if completes {
            peer_reachability.permits.add_permits(1);
        } else {
        ...
        assert!(
            refresh.is_finished(),
            "the complete target operation exceeded twenty seconds"
        );
        let report = refresh.await.unwrap().unwrap();
        assert_eq!(
            (report.online, report.errors),
            if completes { (1, 0) } else { (0, 1) }
        );
...
#[tokio::test(start_paused = true)]
async fn healthy_peers_are_checked_without_user_operations() {
    let (owner, _, peer_reachability) = fixture();
    peer_reachability.reachable.store(true, Ordering::SeqCst);
    owner.start().await;
    settle().await;
    ...
    settle().await;
    let calls = peer_reachability.calls.load(Ordering::SeqCst);
    owner.shutdown().await.unwrap();
    assert_eq!(calls, 2, "online peers need a bounded background check");
}
...
async fn startup_and_later_online_need_no_refresh() {
    ...
    assert_eq!(
        peer_reachability
            .current_state(&DeviceId::new("peer"))
            .await,
        ReachabilityState::Online
    );
    owner.shutdown().await.unwrap();
...
async fn empty_scope_is_woken_when_a_member_becomes_eligible() {
    let (owner, scope, peer_reachability) = fixture();
    scope.peers.lock().await.clear();
    owner.start().await;
    settle().await;
    assert_eq!(peer_reachability.calls.load(Ordering::SeqCst), 0);
...
async fn pause_stops_retries_and_resume_restarts_them() {
    ...
    settle().await;
    assert_eq!(peer_reachability.calls.load(Ordering::SeqCst), calls);
    owner.resume().await.unwrap();
    settle().await;
    assert!(peer_reachability.calls.load(Ordering::SeqCst) > calls);
    owner.shutdown().await.unwrap();
}
...
async fn slow_peers_are_bounded_and_do_not_starve_the_rest() {
    ...
    *scope.peers.lock().await = (0..8).map(|i| DeviceId::new(format!("peer-{i}"))).collect();
    ...
    owner.start().await;
    settle().await;
    assert_eq!(peer_reachability.active.load(Ordering::SeqCst), 4);
    tokio::time::advance(Duration::from_secs(20)).await;
    settle().await;
    assert_eq!(peer_reachability.calls.load(Ordering::SeqCst), 8);
    assert_eq!(peer_reachability.maximum.load(Ordering::SeqCst), 4);
    owner.shutdown().await.unwrap();
    assert_eq!(peer_reachability.active.load(Ordering::SeqCst), 0);
}
...
async fn retry_never_sleeps_forever_and_healthy_connection_is_reused() {
    ...
    for _ in 0..12 {
        ...
        settle().await;
        assert!(peer_reachability.calls.load(Ordering::SeqCst) > before);
    }
    peer_reachability.reachable.store(true, Ordering::SeqCst);
    tokio::time::advance(Duration::from_millis(72_001)).await;
    settle().await;
...
async fn environment_hints_reset_backoff_but_unknown_discovery_does_not() {
    ...
    settle().await;
    assert_eq!(peer_reachability.calls.load(Ordering::SeqCst), before);
    sender
    ...
    tokio::time::advance(Duration::from_secs(80)).await;
    settle().await;
    assert!(peer_reachability.calls.load(Ordering::SeqCst) > before + 1);
    owner.shutdown().await.unwrap();
}

#[tokio::test(start_paused = true)]
async fn lagged_peer_reachability_consumer_reconciles_without_a_busy_loop() {
    let (owner, _, peer_reachability) = fixture();
    for _ in 0..1000 {
        let _ = peer_reachability.events.send(PeerReachabilityChanged {
...
async fn scope_unavailability_during_dial_cancels_before_late_success() {
    ...
    settle().await;
    assert_eq!(peer_reachability.active.load(Ordering::SeqCst), 0);
    assert!(peer_reachability.states.lock().await.is_empty());
    owner.shutdown().await.unwrap();
}
...
async fn fresh_online_before_attempt_timeout_settles_refresh_as_online() {
    ...
    assert_eq!(
        peer_reachability.calls.load(Ordering::SeqCst),
        calls + 1,
        "a healthy newly connected peer still receives its periodic check"
    );
    owner.shutdown().await.unwrap();
}
...
async fn fresh_success_then_disconnect_does_not_hide_attempt_timeout() {
    ...
    for state in [ReachabilityState::Online, ReachabilityState::Offline] {
        peer_reachability.states.lock().await.insert(peer, state);
        peer_reachability
            .events
            .send(PeerReachabilityChanged {
                device_id: peer,
...
#[tokio::test(start_paused = true)]
async fn recovery_confirmation_can_finish_after_ten_seconds() {
    let (owner, _, peer_reachability) = fixture();
    peer_reachability.blocked.store(true, Ordering::SeqCst);
    peer_reachability.reachable.store(true, Ordering::SeqCst);
    owner.start().await;
    ...
    settle().await;
    let report = refresh.await.unwrap().unwrap();
    assert_eq!((report.online, report.errors), (1, 0));
    owner.shutdown().await.unwrap();
}
...
async fn incoming_success_does_not_strand_a_queued_manual_refresh() {
    ...
    *scope.peers.lock().await = (0..5)
    ...
    owner.start().await;
    settle().await;
    assert_eq!(peer_reachability.calls.load(Ordering::SeqCst), 4);
    let refresh = tokio::spawn({
        let owner = owner.clone();
        async move { owner.refresh().await }
    });
    ...
    peer_reachability
        ...
        .unwrap();
    settle().await;
    peer_reachability.permits.add_permits(5);
    settle().await;
