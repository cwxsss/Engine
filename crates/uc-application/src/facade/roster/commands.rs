...
//!
//! * 没有 `ListWithPeerReachabilityQuery` —— 当前没有任何过滤维度可暴露(单 space
//!   场景下返回"全部成员"是唯一语义)。plan §4.1 把它列为未来扩展点,等
//!   第一个过滤条件(按在线状态 / 按 fingerprint 前缀)出现再加,避免现
//!   在硬塞一个空 struct。
//! * 没有 `last_seen_at` —— 该字段需要 `PeerReachabilityPort` 持久追踪"最后一次
//!   Online 的时间戳",但 Slice 2 Phase 1 的 peer_reachability port 只暴露
...
pub struct PeerReachabilityRefreshReport {
    pub total: usize,
    pub online: usize,
    pub offline: usize,
    pub errors: usize,
