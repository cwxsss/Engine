//! `MemberRosterFacade` — 成员名单的对外入口。
//!
//! 对 UI / CLI 暴露已配对成员的列表,并给每条成员打上 peer_reachability 状态
...
//!
//! ## 为什么 `list_with_peer_reachability` 不拨号
//!
//! 查询路径被频繁调用(CLI `members` 每次都会跑,未来 GUI 会做轮询或
//! subscribe)。如果每次查询都调 `peer_reachability.ensure_reachable()`,每个 peer
//! 都会产生一次 iroh dial,浪费带宽且让 UI 响应慢。`peer_reachability.current_state`
//! 是纯缓存读,O(1) 不触发 IO——查询路径应该总是走这一条。
//!
//! ## 模块导出
//!
//! 按 `docs/design-docs/layers/application.md`,只暴露 Facade + Query/Result/Error +
//! 订阅事件类型。`PeerReachabilityChanged` 从 `uc-core` 透出以便上层 crate 订阅
//! `subscribe_peer_reachability_events()` 时不用直接依赖 `uc-core::ports`。
...
/// Map the connection channel to its stable public representation.
